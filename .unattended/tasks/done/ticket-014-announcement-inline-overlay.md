...
> 父 spec：SPEC-012 · 状态：done · 优先级：P1
...
## 验收
- [x] fanyi.lua 启动后 OVERLAY_WIDGETS 注册 announcement widget
- [x] `fanyi status` 输出包含 `announcement_inline (on/off)`
- [x] 重启游戏 → 建要塞 → 任意公告（eg 钓鱼/公鸡/天气变化）→ 公告面板矩形内显示中文
- [x] 90s 后历史公告自动消失
- [x] 单测：缓存 get/set/expire 测试通过（与 013 共用测试组件）
- [x] hack harness 单元测试：`AnnouncementInline:onRenderFrame(dc, 40, 10)` 在 mock announcement 数据下输出非空
- [x] 提交 + 推送
